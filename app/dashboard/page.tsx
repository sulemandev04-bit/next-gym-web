  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white flex">
      ...
      <aside className="hidden md:flex w-64 bg-zinc-900 border-r border-white/5 flex-col p-6 sticky top-0 h-screen">
        ...
        <button
          onClick={logOut}
          className="flex items-center gap-3 text-zinc-500 hover:text-red-500 font-bold uppercase italic text-sm transition-colors mt-auto"
        >
        ...
        <div className="mb-10 flex justify-between items-end">
          <div>
            ...
            <p className="text-zinc-500 text-sm font-bold uppercase tracking-widest mt-1">
              Manage your gym members and inquiries
            </p>
          </div>
        ...
        <div className="space-y-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="bg-zinc-800 text-zinc-400 text-[10px] px-3 py-1 font-bold uppercase">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  {profiles.map((profile, idx) => (
                    <ProfileRow key={profile.id || idx} {...profile} onDelete={deleteProfile} />
                  ))}
                  {!loading && profiles.length === 0 && (
                    <tr>
                      <td colSpan={3} className="p-10 text-center text-zinc-600 italic">
                        No members found.
                      </td>
                    </tr>
                  )}
...
function SidebarLink({ icon, label, active = false }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <Link
      href="#"
      className={`flex items-center gap-4 px-4 py-3 font-bold uppercase italic text-sm transition-all ${
        active ? "bg-orange-600 text-white" : "text-zinc-500 hover:text-white hover:bg-zinc-800"
      }`}
    >
      {icon} {label}
    </Link>
...
function ProfileRow({
  ...
  return (
    <tr className="hover:bg-white/5 transition-colors">
      ...
      <td className="p-4 text-xs font-black italic text-orange-500 uppercase tracking-widest">{role || "Member"}</td>
...
function MessageRow({ name, email, subject, message, created_at }: any) {
  ...
  return (
    <tr className="hover:bg-white/5 transition-colors">
      ...
      <td className="p-4 text-right space-x-2">
        <button className="p-2 text-zinc-400 hover:text-orange-500 hover:bg-orange-500/10 transition-all inline-block">
          <Edit2 size={16} />
        </button>
        <button className="p-2 text-zinc-400 hover:text-red-500 hover:bg-red-500/10 transition-all inline-block">
          <Trash2 size={16} />
        </button>
      </td>
    </tr>
